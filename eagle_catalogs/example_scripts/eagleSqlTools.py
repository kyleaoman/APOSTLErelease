...
class WebDBConnection:
    def __init__(self, username, database="virgodb", password=None):
    ...
    def execute_query(self, sql):
        ...
        # Skip SQL, MAXROWS, QUERYTIMEOUT, QUERYTIME
        for _ in range(4):
            response.readline()
        ...
        return np.loadtxt(response, dtype=dtype, delimiter=",")

    def fetch_docs(self, database, table):
        """Return a list of strings containing the documentation page for the specified table"""
        url = self.db_url + "/Help?" + urllib.urlencode({'page': "databases/"+database+"/"+table})
        urllib2.install_opener(urllib2.build_opener(self.auth_handler, self.cookie_handler))
        response = urllib2.urlopen(url)
        cookie_jar.save(ignore_discard=True)
...
def connect(user, password=None, server="eagle"):
    """Connect to EAGLE database and return a connection object"""
    return WebDBConnection(user, server, password)
